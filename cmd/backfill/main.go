...
type tableSpec struct {
	...
	assemble func(src []any) (row []any, wm int64, skip bool)
}
...
func main() {
	...
	if *mode != "full" && *mode != "delta" && *mode != "final" {
		log.Fatal("--mode must be full, delta, or final")
	}
	full := *mode == "full"
	if !full && *since <= 0 {
	...
	for _, spec := range specs {
		...
		log.Printf("  %-24s synced=%-7d skipped=%-5d maxWatermark=%d", spec.d1Table, n, skipped, maxWM)
	}

	if globalMaxWM > 0 {
		log.Printf("DONE. Next run: --mode=delta --since=%d", globalMaxWM)
	} else {
...
func syncTable(ctx context.Context, conn *pgx.Conn, d1 *d1Client, spec tableSpec, full bool, since float64, batchParams int) (int, int, int64, error) {
...
// toInt64 normalizes pgx numeric scan results (int32/int64) to int64; nil → 0.
...
func buildSpecs(cell, homeCell, blobPrefix, sandboxStates string, keyring *crypto.KeyRing, reenc *crypto.Encryptor) []tableSpec {
	...
	return []tableSpec{
			...
			query: func(full bool) (string, []any) {
				return where(full, `
					SELECT id::text, name, slug, plan, stripe_customer_id, stripe_subscription_id,
					       ...
					       EXTRACT(EPOCH FROM updated_at)::bigint AS _wm
					  FROM orgs`,
					`WHERE updated_at > to_timestamp($1)`), nil
			},
			assemble: func(s []any) ([]any, int64, bool) {
				wm := toInt64(s[27])
				row := []any{
		...
		// users --------------------------------------------------------------
		{
			...
			assemble: func(s []any) ([]any, int64, bool) {
				return []any{s[0], s[1], s[2], s[3], s[4]}, toInt64(s[5]), false
			},
		},
		...
		// org_memberships (derived from users.org_id/role) ------------------
		{
			...
			assemble: func(s []any) ([]any, int64, bool) {
				return []any{s[0], s[1], s[2], s[3]}, toInt64(s[4]), false
			},
		},
		...
		// api_keys -----------------------------------------------------------
		{
			...
			assemble: func(s []any) ([]any, int64, bool) {
				return []any{s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7], s[8], s[9]}, toInt64(s[10]), false
			},
		},
		...
		// secret_stores ------------------------------------------------------
		{
			...
			assemble: func(s []any) ([]any, int64, bool) {
				return []any{s[0], s[1], s[2], s[3], s[4], s[5]}, toInt64(s[6]), false
			},
		},
		...
		// secret_store_entries (BYTEA → BLOB literal) ------------------------
		{
			...
			assemble: func(s []any) ([]any, int64, bool) {
				...
				return []any{s[0], s[1], s[2], ev, s[4], s[5], s[6]}, toInt64(s[7]), false
			},
		},
		...
		// agent_subscriptions (shape mismatch) -------------------------------
		{
			...
			assemble: func(s []any) ([]any, int64, bool) {
				return []any{s[0], s[1], s[2], s[3], s[4], s[5], s[6], s[7]}, toInt64(s[8]), false
			},
		},
		...
		// org_subscription_items (memory_mb → tier) --------------------------
		{
			...
			assemble: func(s []any) ([]any, int64, bool) {
				return []any{s[0], s[1], s[2], ""}, toInt64(s[3]), false
			},
		},
			...
			query: func(full bool) (string, []any) {
				return where(full, `
					SELECT c.id::text, c.sandbox_id, c.org_id::text, c.rootfs_s3_key, c.size_bytes,
					       ...
					       EXTRACT(EPOCH FROM c.updated_at)::bigint AS _wm
